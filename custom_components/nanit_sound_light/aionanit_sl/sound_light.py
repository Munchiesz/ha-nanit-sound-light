...
_MAX_MSG_SIZE: int = 1_048_576  # 1 MiB — reject oversized frames on an untrusted channel
...
# Verbose TX hex dumps are off by default — set NANIT_SL_LOG_TX_HEX=1 to enable
# for wire-format debugging. Command frames don't carry cloud bearer tokens,
# but they're privacy-invasive and noisy.
_LOG_TX_HEX: bool = os.environ.get("NANIT_SL_LOG_TX_HEX", "").lower() in ("1", "true", "yes")
...
class NanitSoundLight:
    ...
    async def _async_fetch_device_token(self) -> None:
        ...
        Inlined (does not rely on NanitRestClient helper methods) so this
        works against any installed aionanit version — mirrors the
        lesson learned in the diagnostics probe fix.
        """
        access_token = await self._token_manager.async_get_access_token()

        # Headers mirror the NanitLite app's exact request shape.
        headers = {
        ...
        async with self._session.get(
            f"{_NANIT_API_BASE_URL}/speakers/{self._speaker_uid}/udtokens",
            headers=headers,
            ...
            if resp.status != 200:
                ...
                raise NanitConnectionError(
                    f"HTTP {resp.status} fetching udtokens for {self._speaker_uid}: "
                    f"{body_text[:200]}"
                )
    ...
    async def _async_close_ws(self) -> None:
        ...
        Excludes the calling task from the cancel list so that
        ``_reconnect_loop`` doesn't cancel its own task mid-iteration
        (which would abort the reconnect at the next await point).
        """
        ...
        for task_attr in ("_recv_task", "_reconnect_task"):
            ...
            if not task.done():
                ...
                try:
                    ...
                    pass
            # Only clear the attribute if it still points to the task we
            # just cancelled (avoids racing with a concurrent re-spawn).
    ...
    async def _async_send(self, data: bytes) -> None:
        ...
        if _LOG_TX_HEX:
            _LOGGER.debug(
                "S&L %s TX %s via %s (%d bytes)",
                self._speaker_uid,
                data.hex(),
                "cloud" if self._use_cloud_relay else "local",
                len(data),
            )
        else:
            _LOGGER.debug(
                "S&L %s TX %d bytes via %s",
                self._speaker_uid,
                len(data),
                "cloud" if self._use_cloud_relay else "local",
            )
    ...
    def _on_message(self, data: bytes) -> None:
        ...
        # Other error responses (e.g. 400 "Failed to parse request"). Logged
        # at WARNING so users can see why commands might not take effect.
        relay_error = extract_cloud_relay_error(data)
        if relay_error is not None and relay_error[0] not in (200, 403):
            status, text = relay_error
            _LOGGER.warning(
                "Sound & Light %s received %d from cloud relay: %s",
                self._speaker_uid,
                status,
                text or "<no text>",
            )
        ...
        try:
            while not self._stopped:
                ...
                try:
                    ...
                    # availability reflects reality and the reconnect loop
                    # (if any) can pick it up. Without this, _connected
                    # stays True with a dead transport and commands error.
                    if self._connected:
                        self._connected = False
                        self._fire_event(SoundLightEventKind.CONNECTION_CHANGE)
        except asyncio.CancelledError:
