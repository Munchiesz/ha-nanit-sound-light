...
def is_cloud_relay_ack(data: bytes) -> bool:
    ...
    receiving a command. Matches ONLY this exact nested structure so we
    don't silently swallow unrelated messages that happen to carry a
    field-3 LENGTH_DELIMITED payload.
    """
    try:
        outer = decode_fields(data)
        f3 = get_field(outer, 3)
        if f3 is None or f3.wire_type != LENGTH_DELIMITED:
            return False
        inner = decode_fields(f3.value)
        f1 = get_field(inner, 1)
        if f1 is None or f1.wire_type != LENGTH_DELIMITED:
            return False
        inner2 = decode_fields(f1.value)
        f1_inner = get_field(inner2, 1)
        return f1_inner is not None and f1_inner.wire_type == VARINT and f1_inner.value == 1
    except Exception:
        return False


def extract_cloud_relay_error(data: bytes) -> tuple[int, str] | None:
    """Pull (status_code, status_text) out of a cloud-relay error envelope.

    Returns None if the buffer does not look like a cloud-relay envelope.
    Used by sound_light.py to surface command rejections in HA logs so we
    can diagnose why certain commands (brightness, color, etc.) are not
    ...
    try:
        ...
        if f_status is None or f_status.wire_type != VARINT:
            return None
        status_code = int(f_status.value)
        f_text = get_field(inner, 3)
        status_text = ""
        if f_text is not None and f_text.wire_type == LENGTH_DELIMITED:
            status_text = try_decode_string(f_text.value) or ""
